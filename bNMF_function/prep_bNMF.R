...
fetch_summary_stats <- function(df_variants, gwas_ss_file, trait_ss_files, pval_cutoff=1) {
  ...
  read_single_trait <- function(trait, variant_df) {
    ...
    df %>%
      filter(VAR_ID %in% variant_df$VAR_ID) %>%
      separate(VAR_ID, into=c("CHR", "POS", "REF", "ALT"), sep="_") %>%
      mutate(SNP=paste(CHR, POS, sep=":")) %>%
      dplyr::select(SNP, Effect_Allele_PH, N_PH, BETA, SE, P_VALUE,REF,ALT) %>%
  ...
  if (class(gwas_ss_file)[1] =="character"){
  ...
  variant_df <- df_variants %>%
    ...
    separate(VAR_ID, into=c("CHR", "POS", "REF", "ALT"), sep="_", remove = F) %>%
    mutate(SNP=paste(CHR, POS, sep=":")) %>%
    dplyr::select(VAR_ID, SNP, Risk_Allele_Orig) %>%
  ...
  variant_df <- variant_df %>% 
    dplyr::select(VAR_ID, SNP, Risk_Allele, Nonrisk_Allele)
...
prep_z_matrix <- function(z_mat, N_mat,
  ...
  # print(medN_vec)
  # print("_______")
  # print(mean(sqrt(medN_vec)))
  # print("_______")
  # print(sqrt(N_mat[, colnames(z_mat)]))
  # print("_______")
  # print(z_mat)
  ...
  if (method_fill=="median"){
    ...
    if (nrow(df_covers)>0) {
      ...
      df_covers <- df_covers %>%
        ...
        dplyr::rename(orig_VAR_ID=VAR_ID, VAR_ID=proxy_VAR_ID)
